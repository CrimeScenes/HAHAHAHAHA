...
local AllBodyParts = {
    ...
    'UpperTorso', 'LowerTorso', 'HumanoidRootPart', 'LeftUpperArm', 'LeftLowerArm',
    'RightUpperArm', 'RightLowerArm', 'LeftHand', 'RightHand', 'LeftUpperLeg',
    'LeftLowerLeg', 'RightUpperLeg', 'RightLowerLeg', 'LeftFoot', 'RightFoot'
}
...
local function UpdateFOV()
    ...
    Circle.Visible = getgenv().Forbidden.CamLock.Normal.RadiusVisibility -- Updated to new table
    Circle.Radius = getgenv().Forbidden.CamLock.Normal.Radius
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == getgenv().Forbidden.CamLock.Normal.Keybind:lower() then
        if getgenv().Forbidden.CamLock.Normal.Enabled then
            if getgenv().Forbidden.CamLock.Normal.Mode:lower() == "toggle" then -- Changed from 'mode' to 'Mode'
            ...
            elseif getgenv().Forbidden.CamLock.Normal.Mode:lower() == "hold" then -- Changed from 'mode' to 'Mode'
                IsTargeting = true
                TargetPlayer = ClosestPlrFromMouse()
            end
        end
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        local BodyPart
        if getgenv().Forbidden.CamLock.Normal.ClosestPart then
            BodyPart = GetClosestBodyPart(TargetPlayer.Character)
        else
            BodyPart = TargetPlayer.Character:FindFirstChild(getgenv().Forbidden.CamLock.Normal.HitPart)
        end

        if BodyPart then
            local predictedPosition
            if getgenv().Forbidden.CamLock.Normal.Resolver then
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * getgenv().Forbidden.CamLock.Normal.Prediction)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * getgenv().Forbidden.CamLock.Normal.Prediction)
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if getgenv().Forbidden.CamLock.Normal.SmoothnessEnabled then
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, getgenv().Forbidden.CamLock.Normal.Smoothness)
                else
...
local function isPartInFovAndVisible(part)
    ...
    return onScreen and distance <= getgenv().Forbidden.Silent.Normal.FovSettings.Radius
...
local OldPredictionY = getgenv().Forbidden.Silent.Normal.PredictionValue -- Updated from Prediction
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if velocity.Y < -30 and getgenv().Forbidden.Silent.Normal.Resolver then
            getgenv().Forbidden.Silent.Normal.PredictionValue = 0 -- Updated from Prediction
            return velocity
        elseif velocity.Magnitude > 50 and getgenv().Forbidden.Silent.Normal.Resolver then
            ...
            getgenv().Forbidden.Silent.Normal.PredictionValue = OldPredictionY -- Updated from Prediction
            return velocity
        end
    end
...
local function toggleFeature()
    ...
    if not getgenv().Forbidden.Silent.Normal.Enabled then
        getgenv().Forbidden.Silent.Normal.FovSettings.Visible = false -- Updated from Fov.Visible
        clearFovParts()
    end
end
...
Run_Service.RenderStepped:Connect(function()
    if getgenv().Forbidden.Silent.Normal.Enabled and IsTargeting then
        ...
        if Target and Target.Character then
            ...
            if closestPart and closestPoint then
                ...
                Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().Forbidden.Silent.Prediction.Value)
...
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().Forbidden.Silent.Normal.Enabled then
            ...
            Fov.Visible = IsTargeting and getgenv().Forbidden.Silent.Normal.FovSettings.Visible  -- Update visibility based on targeting
        end
    end
end)
    ...
    if tool:IsA("Tool") then
        tool.Activated:Connect(function()
            if Target and Target.Character and tick() - lastToolUse > 0.1 then  -- Debounce for 0.1 seconds
                ...
                if closestPart and closestPoint then
                    local velocity = GetVelocity(Target, closestPart.Name)
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().Forbidden.Silent.Normal.Prediction.Value)
...
if getgenv().Forbidden.Adjustment.Checks.NoGroundShots == true then
    local function CheckNoGroundShots(Plr)
        if getgenv().Forbidden.Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(getgenv().Forbidden.Silent.Normal.Enabled and getgenv().Forbidden.Silent.Normal)
